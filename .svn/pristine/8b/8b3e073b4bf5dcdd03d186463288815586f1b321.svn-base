...
$(document).ready(function() {
	product = $.getUrlParam('numberCode');
	addProductStock();
	dateArr = new Array();
	sEventCurrent = false;
...
function initTable(){
	...
	xyzgrid({
		...
		url:'../ProductStockWS/queryProductStockList.do',
		toolbar : toolbar,
		title:'管理库存',
		height:'auto',
		singleSelect :false,
		columns:[[
		    ...
		    {field:'stockTempCount',title:'占位数量'},
...
function addProductStockSubmit(){
	...
	if(stockCommboxForm=="+"){
		...
		layer.msg("请在下拉框选择正确的加减库存选项！", {icon: 0});
		return false;
	}
...
function queryProductStockForCalendarButton(){
	...
	if(new Date(queryDate)<currerentDate.getTime()){
		layer.msg("不能小于当前日期哦！", {icon: 5});
		return;
	}
	if(xyzIsNull(queryDate)){
		layer.msg("请选择查询日期！", {icon: 0});
		return;
	}else{
