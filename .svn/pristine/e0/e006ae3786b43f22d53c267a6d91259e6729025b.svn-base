...
function initTable(){
	...
	/*toolbar[toolbar.length]={
			text: '求助范围地图',
			iconCls: 'icon-edit',
			handler: function(){showSeekTagsHelpMap();}
	};*/
		...
		columns:[[
			    ...
			    {field:'headimgurl',title:'头像',width:30,
					formatter : function(value,row,index){
						var info = JSON.parse(row.openidAppend);
						return '<img width="30px" height="30px" src="'+info.headimgurl+'" ondblclick="window.open(this.src);" title="双击打开大图" />';
					}
				},
				...
				{field:'location',title:'地图中查看',
					formatter : function(value,row,index){
						var a = '<a href="javascript:void(0);" onclick="showSeekHelpMap(\''+row.locationX+'\',\''+row.locationY+'\',\''+row.label+'\')">地图中查看</a>';
						return a;
					}
				},
...
function showSeekHelpMap(x,y,label){
	xyzdialog({
		dialog : 'dialogFormDiv_showSeekHelpMap',
	    href : '../jsp_core/showSeekHelpMap.html',
	    title:"地图坐标",
	    buttons:[{
			text:'取消',
			handler:function(){
				$("#dialogFormDiv_showSeekHelpMap").dialog("destroy");
			}
		}],onOpen:function(){
			var ex = /^\d+$/;
...
function showSeekTagsHelpMap(){
	xyzdialog({
	    ...
	    buttons:[{
			...
			var latXandLatY = getLatXandLatY();
			var ex = /^\d+$/;
			var geocoder = new qq.maps.Geocoder();
			var center = new qq.maps.LatLng(39.916527, 116.397128);
			var map = new qq.maps.Map(document.getElementById("container"), {
			...
			for(var i=0;i<latXandLatY.length;i++){
				(function(n){
					var num=0;
					var x=latXandLatY[i].locationX;
					var y=latXandLatY[i].locationY;
					if (ex.test(x)) {
						num=num+1;
					}
					if (ex.test(y)) {
