			...
			if (hasPulldown || hasPullup) {
				...
				if (container) {
					...
					if ($container.length === 1) {
						if (false/**$.os.plus && $.os.android**/) { //android 5+
							$.plusReady(function() {
								...
								if (hasPulldown) {
									...
									if (parent) {
										...
										//父页面初始化pulldown
										parent.evalJS("mui&&mui(document.querySelector('.mui-scroll-wrapper')).pullRefresh('" + JSON.stringify(downOptions) + "')");
									}
								}
							});
						} else {
...
///这里定义过pullRefresh 5+安卓专用function
/**
 * off-canvas
 * @param {type} $
 * @param {type} window
