...
(function(thiz){
	...
	/*	server : 'http://localhost:8080/xztrip/',*/
	...
	xyz.getMainWebview=function(){
		var view=plus.webview.getWebviewById(plus.os.name=='iOS'?'main':'Xztrip');
	...
	xyz.ajax = function(p){
		...
		mui.ajax(p.url?(p.url.indexOf('http')==0?p.url:xyz.config.server+p.url):'',{
			data:p.data?p.data:{},
			dataType:'json',
			type:'post',
			timeout:20000,//超时
