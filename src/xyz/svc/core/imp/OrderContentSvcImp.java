...
public class OrderContentSvcImp implements OrderContentSvc {
	...
	public int pushOrdertoErpOper() {
		...
		String hql="from OrderContent where flagClient=0 and flagPay=1 and flagDelete=0 and flagUse=0 and flagRefund=0 and isException=0 ";
		@SuppressWarnings("unchecked")
		...
		for(OrderContent orderContent:orders){
			...
			accessoryParam.put("providerNameCn", orderContent.getProviderNameCn());
			accessoryParam.put("tkviewNameCn", orderContent.getProductNameCn());
			...
			if(status==1){
				...
				exceptionLogSvc.addOrderException(orderContent.getOrderNum(), ExceptionLog.TYPE_ERP, JSON.toJosn(result));
			}
		}
	...
	@Override
	public int rollbackOverOrderOper() {
		
		String hql="from OrderContent where flagPay=0 and flagOver=0 GROUP BY orderNum";
		...
		for(OrderContent orderContent:orderContents){
			...
			//支付超时回退库存
			if(orderContent.getOverTime()!=null&&orderContent.getOverTime().getTime()>now){
				buyerOrderSvc.cancelPmsOrder(orderContent.getOrderNum());
