...
//		long timeexpire = timeStart+1800000;
		long timeexpire = timeStart+300000;
...
//		wxBill.setTotal_fee(1);
	...
	@SuppressWarnings("unchecked")
	@Override
	public Map<String, Object> addWxBillByJs(WxBill wxBill) {
		
		XyzSessionLogin login = MyRequestUtil.getXyzSessionLogin();
...
//		long timeexpire = timeStart+1800000;
		long timeexpire = timeStart+300000;
		wxBill.setAppid(WxJsPayUtil.corpid_js);
		wxBill.setAttach("");
		wxBill.setBillCreateTime(new Date());
		wxBill.setBillNo(uuid);
		wxBill.setBillStatus(0);
//		wxBill.setBody("444");
		...
		wxBill.setGoods_tag("");
		wxBill.setLimit_pay("");
		wxBill.setMch_id(WxJsPayUtil.mch_id_js);
		wxBill.setNonce_str(WxJsPayUtil.getNonceStr(16));
		wxBill.setNotify_url(WxJsPayUtil.WXPAY_NOTFIY_URL);
		/***********************************************注意openid，部署正式服要使用用户的openid************************************************************/
		wxBill.setOpenid(login!=null?login.getOpenid():"");
//		wxBill.setOpenid("owK5Ws-yQznRAzEN61JUGtv1bX_M");
		/***********************************************注意openid，部署正式服要使用用户的openid************************************************************/
		wxBill.setOut_trade_no(uuid);
		wxBill.setProduct_id("");
		wxBill.setSign("");
//		wxBill.setSpbill_create_ip("192.168.1.10");
		...
		wxBill.setTime_start(sdf.format(new Date(timeStart)));
//		wxBill.setTotal_fee(1);
		wxBill.setTrade_type("JSAPI");
		
		//创建订单
		commonDao.save(wxBill);
		
		Map<String, Object> payResult = WxJsPayUtil.doWxJsPay(wxBill);
		if("1".equals(payResult.get("status").toString())){
			payResult = (Map<String, Object>)payResult.get("content");
			String prepayid = payResult.get("prepay_id")==null?null:payResult.get("prepay_id").toString();
			if(prepayid!=null && !"".equals(prepayid)){
				SortedMap<String, String> prepayInfo = WxJsPayUtil.wxPayRequest2WxWebApp(prepayid);
				return ReturnUtil.returnMap(1, prepayInfo);
			}
			return ReturnUtil.returnMap(0, "服务器异常");
		}
