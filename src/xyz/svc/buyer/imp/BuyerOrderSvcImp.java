...
import xyz.svc.config.ConstantOrderClose;
...
public class BuyerOrderSvcImp implements BuyerOrderSvc {
	...
	public Map<String, Object> createOrder(String product,
		...
		//计算订单支付失效时间
		long time=new Date().getTime()+ConstantOrderClose.CLOSE_TIME;
		...
		for(int i=0;i<day;i++){
			...
			orderContent.setOverTime(new Date(time));
			...
			orderContent.setFlagOver(0);//订单正常，支付未失效
	...
	public Map<String, Object> holdStockOper(String orderNum) {
		...
		@SuppressWarnings("unchecked")
		List<OrderContent> orderContentList = commonDao.queryByHql("FROM OrderContent WHERE orderNum = '"+orderNum+"' AND isException = 0 AND flagPay = 1");
	...
	public Map<String, Object> rollbackStockOper(String orderNum) {
		...
		if(orderContentList.size()>0){
			...
			//如果是PMS订单，则到PMS取消占位订单并还原PMS库存
			if(productUtil.isPmsProduct(productObject.getNumberCode())){
				Map<String, Object> resultMap=cancelPmsOrder(orderContentList.get(0).getOrderNum());
				if((Integer)resultMap.get(Constant.result_status)!=1){
					return resultMap;
				}
			}else{
				for(int i=0; i<orderContentList.size(); i++){
					OrderContent orderContent = orderContentList.get(i);
					//回退库存（取消库存）
					stockPriceUtil.rollbackStock(orderContent.getClientCode());
					//改变发起支付状态和时间
					//orderContent.setFlagPayPlan(1);
					//orderContent.setPayPlanDate(date);
				}
	...
	public Map<String, Object> createPmsOrder(String orderNum){
		...
		if(orderList.size()>0){
			...
			if((Integer)resultMap.get(Constant.result_status)!=1){
				exceptionLogSvc.addOrderException(orderNum, ExceptionLog.TYPE_PMS, JSON.toJosn(resultMap));
			}
			return resultMap;
		}
	...
	//取消PMS占位订单，回退库存
		...
		public Map<String, Object> cancelPmsOrder(String orderNum){
			...
			if(orderList.size()>0){
				...
				Map<String,String> accessoryParam = new HashMap<String, String>();
				accessoryParam.put("orderNum", orderNum);
				accessoryParam.put("payMethod", "网银或移动端");
				
				@SuppressWarnings("unchecked")
				Map<String, Object> resultMap=(Map<String, Object>) rmiUtil.loadData(ConstantPms.pms_cancelPmsOrder,accessoryParam);
				if((Integer)resultMap.get(Constant.result_status)!=1){
					exceptionLogSvc.addOrderException(orderNum, ExceptionLog.TYPE_PMS, JSON.toJosn(resultMap));
				}
				return resultMap;
	...
	public Map<String, Object> decideOrder(String orderNum, BigDecimal money) {
		...
		if(now>orderContentList.get(0).getOverTime().getTime()){
			rollbackStockOper(orderContentList.get(0).getOrderNum());
			for(OrderContent orderContent:orderContentList){
				orderContent.setFlagOver(1);
				commonDao.update(orderContent);
			}
			return ReturnUtil.returnMap(0, "订单支付超时失效，请重新选择下单");
		}
		...
		for(int i=0; i<orderContentList.size(); i++){
			...
			//检查库存
			if((productStock.getCount()-productStock.getUseCount())<orderContent.getCount()){
	...
	public Map<String, Object> paySuccessOper(String orderNum) {
		...
		String hql="from OrderContent where orderNum='"+orderNum+"'";
		@SuppressWarnings("unchecked")
		List<OrderContent> contents=commonDao.queryByHql(hql);
		if(contents.size()>0){
			if(productUtil.isPmsProduct(contents.get(0).getProduct())){
				modifyOrderPayStatus(contents.get(0).getOrderNum());
			}
		}
		
		String date = DateUtil.dateToString(new Date());
		//变更order_content订单的支付状态
		commonDao.updateByHql("UPDATE OrderContent SET flagPay=1,payDate='"+date+"' WHERE orderNum ='"+orderNum+"'");
	...
	public Map<String, Object> modifyOrderPayStatus(String orderNum) {
		...
		if(orderList.size()>0){
			...
			for(OrderContent orderContent:orderList){
				payMoney.add(orderContent.getMoney());
			}
	
			Map<String,String> accessoryParam = new HashMap<String, String>();
			accessoryParam.put("b2bOrderNum", orderNum);
			accessoryParam.put("payMoney", payMoney.toString());
			
			@SuppressWarnings("unchecked")
			Map<String, Object> resultMap=(Map<String, Object>) rmiUtil.loadData(ConstantPms.pms_modifyOrderPayStatus,accessoryParam);
			if((Integer)resultMap.get(Constant.result_status)!=1){
				exceptionLogSvc.addOrderException(orderNum, ExceptionLog.TYPE_PMS, JSON.toJosn(resultMap));
			}
			return resultMap;
