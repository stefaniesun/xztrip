	...
	**/
	xyzDropzone.create=function(config){
		...
		init({
			...
			params:('params' in config)?config.params:{"derictoryCode":"mydirectory"},
			maxFiles:('maxFiles' in config)?config.maxFiles:10,
	...
	function init(config){
		...
		myDropzone.on("addedfile",function(file){
			...
			if(file.type.match(/image.*/)){//如果上传得是图片
				if('imageSize' in config){//检查是否需要限制图片宽高 需要开启createImageThumbnails
					var result = {"fileName":file.name};
					if(('minWidth' in config.imageSize) && file.width<config.imageSize.minWidth){//最小宽度限制
						this.removeFile(file);
						result.msg = "超过图片最小宽度限制"+config.imageSize.minWidth;
						config["invalidFileFail"](result);
					}else if(('maxWidth' in config.imageSize) && file.width>config.imageSize.maxWidth){//最大宽度限制
						this.removeFile(file);
						result.msg = "超过图片最大宽度限制"+config.imageSize.maxWidth;
						config["invalidFileFail"](result);
					}else if(('minHeight' in config.imageSize) && file.height<config.imageSize.minHeight){//最小高度限制
						this.removeFile(file);
						result.msg = "超过图片最小高度限制"+config.imageSize.minHeight;
						config["invalidFileFail"](result);
					}else if(('maxHeight' in config.imageSize) && file.height>config.imageSize.maxHeight){//最大高度限制
						this.removeFile(file);
						result.msg = "超过图片最大高度限制"+config.imageSize.maxHeight;
						config["invalidFileFail"](result);
					}
				}
			}
		...
		myDropzone.on("maxfilesexceeded",function(file){
			this.removeFile(file);
			config["invalidFileFail"]({"msg":"这里最多允许上传"+('maxFiles' in config)?config.maxFiles:10+"个文件"});
		});
		//进度条
