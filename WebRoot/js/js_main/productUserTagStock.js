...
function addProductUserTagStockSubmit(){
	...
	var discount = $("#discountForm").numberbox("getValue").trim();
	var reduction = $("#reductionForm").numberbox("getValue").trim();
	if(xyzIsNull(discount)&&xyzIsNull(reduction)){
		alert("折扣和减免至少需要设置一个");
		return;
	}
	if(xyzIsNull(discount)){
		discount="10.0";
	}
	if(xyzIsNull(reduction)){
		reduction="0.00";
	}
